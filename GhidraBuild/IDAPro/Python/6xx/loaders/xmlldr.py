...
class XmlImporter:
    ...
    def import_function(self, function):
        ...
        try:
            ...
            if regcmt != None:
                self.update_counter(f'{FUNCTION}:{REGULAR_CMT}')
                idaapi.set_func_cmt(func, regcmt.text, False)
            rptcmt = function.find(REPEATABLE_CMT)
            if rptcmt != None:
                self.update_counter(f'{FUNCTION}:{REPEATABLE_CMT}')
                idaapi.set_func_cmt(func, rptcmt.text, True)
            typecmt = function.find(TYPEINFO_CMT)
            if typecmt != None:
                self.update_counter(f'{FUNCTION}:{TYPEINFO_CMT}')
                idc.SetType(entry_point, f'{typecmt.text};')
