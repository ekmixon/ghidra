...
class XmlExporter:
    ...
    def export_stack_vars(self, function, sframe):
        ...
        for i in range(sframe.memqty):
            ...
            if regcmt != None:
                regcmt = idaapi.tag_remove(f"{regcmt} ", 0)
            if rptcmt != None:
                rptrcmt = idaapi.tag_remove(f"{rptcmt} ", 0)
    ...
    def get_address_string(self, addr):
        ...
        elif (idaapi.ph_get_id() == idaapi.PLFM_386 and
    ...
    def get_data_value(self, addr):
        ...
        return idaapi.get_64bit(addr) if size == 8 else 0
    ...
    def get_datatype(self, addr):
        ...
        if idaapi.isASCII(f) == True:
            ...
            return "unicode4" if str_type == idaapi.ASCSTR_ULEN4 else "string"
        return "pointer" if idaapi.isOff0(f) == True else t
